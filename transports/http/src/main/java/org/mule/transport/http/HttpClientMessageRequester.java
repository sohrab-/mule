...
public class HttpClientMessageRequester extends AbstractMessageRequester
{
     ...
     * 
     * @param timeout the maximum time the operation should block before returning. The call should return
     *            immediately if there is data available. If no data becomes available before the timeout
     *            elapses, null will be returned
     * @return the result of the request wrapped in a MuleMessage object. Null will be returned if no data was
     *         avaialable
    ...
    protected MuleMessage doRequest(long timeout) throws Exception
    {
        ...
        HttpGet httpMethod = new HttpGet(endpoint.getEndpointURI().getAddress());
        ...
        try
        {
            ...
            {
                MuleMessage res = (MuleMessage) receiveTransformer.transform(response);
            ...
            {
                releaseConn = true;
                throw new ReceiveException(HttpMessages.requestFailedWithStatus(response.getStatusLine()
                    .toString()), endpoint, timeout);
            }
        }
